...
class DockerService:
    ...
    # ------------------------------------------------------------------ #
    #  VPN / Compose-stack integration helpers                            #
    # ------------------------------------------------------------------ #
    ...
    def _get_bot_network_mode() -> str:
        ...
        Reads DOCKER_BOT_NETWORK_MODE from the environment.
        Examples:
          container:gluetun   – share the gluetun VPN stack (recommended)
          host                – legacy behaviour
          <empty>             – falls back to "host" for backward compat
        """
        return os.environ.get("DOCKER_BOT_NETWORK_MODE", "host")

    @staticmethod
    def _get_compose_labels(instance_name: str) -> dict:
        """Return Docker labels that graft the bot container into
        the running Compose project so it appears in
        `docker compose ps`, Dozzle, and autoheal.

        Reads:
          COMPOSE_PROJECT_NAME  – set automatically by `docker compose`
                                  inside every container in the project.
          COMPOSE_SERVICE_PREFIX – optional, defaults to "hummingbot-bot"
        """
        ...
        if project:
            # These are the labels `docker compose` itself sets.
            # Adding them makes the container show up in
    ...
    def create_hummingbot_instance(self, config: V2ScriptDeployment):
        ...
        # ── Network mode & stack integration ─────────────────────
